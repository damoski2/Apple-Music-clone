...
export default (state: any, action: any) => {
  ...
  switch (action.type) {
    ...
    case "SET_EXPIRES_IN":
      let expIn;
      if (
        payload.expiresIn?.toString().length > 4 ||
        payload.expires_in?.toString().length > 4
      ) {
        expIn = payload.expiresIn || payload.expires_in;
      } else {
        expIn = millsecondstoISODateconverter(
    ...
    case "SET_PLAY_NEXT_URI":
      return {
        ...state,
        playNextUri: payload,
      };

    case "SET_OPEN_MODAL_SONGURI":
      return {
        ...state,
        openModalSongUri: payload,
      };

    case "SET_GENRE_PLAYLISTS":
      return {
        ...state,
        genrePlaylist: payload,
      };

    case "SET_SEARCH_POPUP_REF":
      return {
        ...state,
        searchPopupRef: payload,
      };
